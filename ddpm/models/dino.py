...
class ViTExtractor(nn.Module):
    ...
    def __init__(self, model_type: str = 'dino_vits8', stride: int = 4, model: nn.Module = None, device: str = 'cuda'):
        ...
        self.p = self.model.patch_embed.patch_size # patch size
    ...
    def extract_descriptors(self, batch: torch.Tensor,
                            layers: Union[int, list] = 11,
                            facet: str = 'key',
                            include_cls: bool = False,
                            resize_shape: Union[tuple, None]= None) -> Union[torch.Tensor, list]:
        """
        ...
        if type(layers) == int:
            self._extract_features(batch, [layers], facet)
            x = self._feats[0]
            if facet == 'token':
            ...
            b = x.shape[0]
            x = x.permute(0, 2, 3, 1).flatten(start_dim=-2, end_dim=-1).unsqueeze(dim=1)  # Bx1xtx(dxh)
            x = x.view(b, 1, self.num_patches[0], self.num_patches[1], -1).squeeze(1).permute(0, 3, 1, 2)
            if resize_shape is None:
                x = torch.nn.functional.interpolate(x, (self.load_size[0]//self.stride[0], self.load_size[1]//self.stride[1]), mode='bilinear')
            else:
                x = torch.nn.functional.interpolate(x, resize_shape, mode='bilinear')
            return x
            ...
            if resize_shape is not None:
                desc = [torch.nn.functional.interpolate(x, resize_shape, mode='bilinear') for x in desc]
            return desc
