...
def evaluateImgLists(predictionImgList, groundTruthImgList, args, transform):
    ...
    for i in range(len(predictionImgList)):
        ...
        nbPixels += evaluatePair(predictionImgFileName, groundTruthImgFileName, confMatrix, instStats, perImageStats, args, transform)
...
def evaluatePair(predictionImgFileName, groundTruthImgFileName, confMatrix, instanceStats, perImageStats, args, transform):
    ...
    predictionNp = transform(predictionNp).numpy()
    try:
    ...
    groundTruthNp = transform(groundTruthNp).numpy()
