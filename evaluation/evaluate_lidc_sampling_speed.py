...
from ddpm.trainer import _build_model
...
class Tester:
    ...
    def test_step(self, _: Engine, batch: Tensor) -> Dict[str, Any]:
        ...
        prediction = self.polyak.average_model(x, image, t=torch.as_tensor(self.K))['diffusion_out']
    ...
    def objects_to_save(self, engine: Optional[Engine] = None) -> Dict[str, Any]:
        to_save: Dict[str, Any] = {
            "average_model": self.polyak.average_model.unet,
        }
...
def eval_lidc_sampling_speed(params):
    ...
    indices = []  # define indices to use for experiement
    test_dataset = dataset_module.test_dataset(max_size=None, indices=indices)  # type: ignore
    ...
    timesteps = [250, 200, 150, 100, 50, 25, 10]

    for timestep in timesteps:
        LOGGER.info(f"Evalute model with sampling step {timestep}...")
        K = 10000 + timestep
        tester = Tester(polyak, K, params["evaluations"], num_classes, np.zeros(len(params["evaluations"])), np.zeros(len(params["evaluations"])), np.zeros(len(params["evaluations"])), np.zeros(len(params["evaluations"])), 0, 0)
